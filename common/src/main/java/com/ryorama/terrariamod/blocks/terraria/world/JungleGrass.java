...
public class JungleGrass extends BlockT {
    public JungleGrass(BlockBehaviour.Properties settings) {
        ...
        if (!world.isClientSide()) {
            ...
            if (world.getBlockState(pos2).getBlock().defaultBlockState() == Blocks.AIR.defaultBlockState()) {
                if (random.nextInt(100 * 10) <= 2) {
                    world.setBlock(pos2, BlocksT.JUNGLE_GRASS.get().defaultBlockState(), 0);
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (world.getBlockState(blockpos.above()).getBlock() == Blocks.AIR) {
                    if (world.getBlockState(blockpos).getBlock() == BlocksT.MUD.get()) {
                        world.setBlock(blockpos, BlocksT.JUNGLE_GRASS.get().defaultBlockState(), 0);
                    }
                }
            }
        }
