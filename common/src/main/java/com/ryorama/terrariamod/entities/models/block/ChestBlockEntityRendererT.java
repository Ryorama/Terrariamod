/*
package com.ryorama.terrariamod.entities.models.block;
...
public class ChestBlockEntityRendererT<T extends ChestBlockEntity> extends ChestBlockEntityRenderer<T> {

    private final ModelPart chestLid;
    private final ModelPart chestBottom;
    private final ModelPart chestLock;

    public ChestBlockEntityRendererT(BlockEntityRendererFactory.Context context) {
        super(context);
    ...
    public void render(T entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (block instanceof ChestT) {
            ...
            matrices.multiply(Direction.UP.getRotationQuaternion().invert());
            matrices.translate(-0.5D, -0.5D, -0.5D);
            ...
            float g = ((Float2FloatFunction)properties.apply(ChestT.getAnimationProgressRetriever((LidOpenable) entity))).get(tickDelta);
            g = 1.0F - g;
            g = 1.0F - g * g * g;
            int i = ((Int2IntFunction)properties.apply(new LightmapCoordinatesRetriever())).applyAsInt(light);

            SpriteIdentifier spriteIdentifier = new SpriteIdentifier(TexturedRenderLayers.CHEST_ATLAS_TEXTURE, chest.getTexture());
