...
public class EntityWyvernHead extends MobEntity implements IEntityAdditionalSpawnData, IHostile {
	   public float velX;
	   public float velY;
	   public float velZ;
	   ...
	   public EntityWyvernHead(EntityType type, World worldIn) {
	      super(type, worldIn);
	   }

	   public EntityWyvernHead(World worldIn) {
	      super(EntitiesT.WYVERN_HEAD, worldIn);
	   }
	   ...
	   public void tick() {
	      ...
	      if (!this.world.isRemote) {
	         ...
	         if (spawnedSegments == false) {
		         if (this.ai[0] == 0) {
		            EntityWyvernBody lastWormBody = null;
	
		            for(int i = 0; i < this.segments; ++i) {
		            	EntityWyvernBody wormBody = EntityWyvernBody.spawnWormBody(this.world, this.getPosition(), this.getMaxHealth(), (MobEntity)(lastWormBody == null ? this : lastWormBody));
		               lastWormBody = wormBody;
		            }
	
		            EntityWyvernTail.spawnWormBody(this.world, this.getPosition(), this.getMaxHealth(), (MobEntity)(lastWormBody == null ? this : lastWormBody));
		            this.ai[0] = 1;
