...
public class Spazmatism extends FlyingEntity implements IEntityAdditionalSpawnData, IHostile {
	...
	public Spazmatism(EntityType<Spazmatism> type, World world) {
		super(type, world);
	    isAlive = true;
	    phase = 1;
	    if (!this.world.isRemote()) {
	         this.world.getServer().getPlayerList().sendMessage((new StringTextComponent("Spazmatism has awoken!")).applyTextStyles(new TextFormatting[]{TextFormatting.BLUE, TextFormatting.BOLD}));
	     }
	}
	
	public Spazmatism(World world) {
	      super(EntitiesT.SPAZMATISM, world);
	}
	
	 @Nullable
	   public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {

	      this.getAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(23000);
	      this.setHealth(23000);
	 ...
	 public void dropLoot(DamageSource source, boolean b) {
		   ...
		   isAlive = false;
		   if (!this.world.isRemote()) {
		         this.world.getServer().getPlayerList().sendMessage((new StringTextComponent("Spazmatism has been defeated!")).applyTextStyles(new TextFormatting[]{TextFormatting.BLUE, TextFormatting.BOLD}));
		     }
	   }
	...
	public void tick() {
        ...
        if (this.getHealth() <= 11500 && transformed == false) {
        	transformed = true;
