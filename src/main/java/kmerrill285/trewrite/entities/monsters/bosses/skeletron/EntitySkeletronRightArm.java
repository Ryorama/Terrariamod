...
public class EntitySkeletronRightArm extends MobEntity implements IEntityAdditionalSpawnData, IHostile {
	 ...
	 public float velX;
	   ...
	   public EntitySkeletronRightArm(EntityType<EntitySkeletronRightArm> type, World worldIn) {
		   super(type, worldIn);
		   isAlive = true;
	   }
	   
	   public EntitySkeletronRightArm(World worldIn) {
		      super(EntitiesT.SKELETRON_RIGHT_ARM, worldIn);
	   }
	       	    ...
	       	    if (!world.isRemote()) {
		  		   if (!spawnedSegments) {
		  			 spawnedSegments = true;
		 		 			EntitySkeletronRightArm2 leftarm = EntitiesT.SKELETRON_RIGHT_ARM2.create(world, null, null, null, new BlockPos(this.getPosition()), SpawnReason.EVENT, false, false);
