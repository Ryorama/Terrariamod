...
public class EntityItemT extends Entity implements IEntityAdditionalSpawnData {
   ...
   public void applyEntityCollision(Entity entityIn) {
      if (this.getItem() != null && this.getItem().item == ItemsT.FALLEN_STAR && !this.hitGround && entityIn instanceof LivingEntity && !(entityIn instanceof PlayerEntity)) {
         entityIn.attackEntityFrom(DamageSource.GENERIC, 2500.0F);
      }
   ...
   public void tick() {
      ...
      if (this.getItem() != null && this.getItem().item == ItemsT.FALLEN_STAR) {
         if (!this.hitGround) {
            for(int i = 0; i < 10; ++i) {
               this.world.addParticle(ParticleTypes.END_ROD, this.posX, this.posY, this.posZ, this.rand.nextDouble() - 0.5D, this.rand.nextDouble(), this.rand.nextDouble() - 0.5D);
            }
         }
                     ...
                     if (dist < 0.25D) {
                        ...
                        if (!world.isRemote) {
                           ...
                           if (!this.dead && !this.removed) {
                              world.playSound((PlayerEntity)null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.PLAYERS, 0.5F, 2.0F + this.rand.nextFloat() * 2.0F - 1.0F);
                           }
