...
public class EntityStar extends MobEntity {
   ...
   public EntityStar(EntityType type, World worldIn) {
      super(type, worldIn);
   }
   ...
   public void tick() {
      ...
      if (this.dead) {
         ...
         if (closest != null && dist < 8.0D && dist <= 2.0D && !this.dead && ScoreboardEvents.getScore(closest.getWorldScoreboard(), closest, ScoreboardEvents.MANA).getScorePoints() < ScoreboardEvents.getScore(closest.getWorldScoreboard(), closest, ScoreboardEvents.MAX_MANA).getScorePoints()) {
   ...
   public static EntityStar spawnStar(World worldIn, BlockPos pos) {
      EntityStar star = (EntityStar)EntitiesT.STAR.create(worldIn, (CompoundNBT)null, (ITextComponent)null, (PlayerEntity)null, pos, SpawnReason.EVENT, false, false);
