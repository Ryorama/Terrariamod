...
public class CorruptHighlandsBiome extends Biome {
   protected CorruptHighlandsBiome() {
      super((new Builder()).surfaceBuilder(new ConfiguredSurfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(BlocksT.CORRUPT_GRASS.getDefaultState(), BlocksT.DIRT_BLOCK.getDefaultState(), BlocksT.EBONSTONE.getDefaultState()))).precipitation(RainType.RAIN).category(Category.EXTREME_HILLS).depth(0.1F).scale(0.5F).temperature(0.2F).downfall(0.3F).waterColor(4159204).waterFogColor(329011).parent((String)null));
