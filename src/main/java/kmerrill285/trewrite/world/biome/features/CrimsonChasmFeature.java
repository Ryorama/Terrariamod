...
public class CrimsonChasmFeature extends Feature<NoFeatureConfig> {	
	
	public CrimsonChasmFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn) {
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, NoFeatureConfig config) {
		...
		if (rand.nextInt(50) == 0) {
			...
			Template template = ((ServerWorld) worldIn.getWorld()).getSaveHandler().getStructureTemplateManager()
					.getTemplateDefaulted(new ResourceLocation("trewrite", "crimson_chasm"));
			if (template == null)
				return false;
