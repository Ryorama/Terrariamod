...
public class RuinedHouseFeature extends Feature<NoFeatureConfig> {	
	
	public RuinedHouseFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn) {
		...
		if (rand.nextInt(900) == 0) {
			i += rand.nextInt(16) + 8;
				...
				Template template = ((ServerWorld) worldIn.getWorld()).getSaveHandler().getStructureTemplateManager()
						.getTemplateDefaulted(new ResourceLocation("trewrite", "ruined_house"));
				if (template == null)
					return false;
