...
public class CrimsonHeart extends CrossedBlock implements IWaterLoggable  {
	...
	public void onReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
		...
		if (!world.isRemote()) {
			...
			if (orbs % 3 == 2) {
				world.addEntity(EntitiesT.BOC.create(world, null, null, null, new BlockPos(Minecraft.getInstance().player.posX + 10, Minecraft.getInstance().player.posY, Minecraft.getInstance().player.posZ + 10), SpawnReason.EVENT, false, false));
			}
	    }
